...
func (r *periodicRunner) Run() RunnerResults {
	...
	var numCalls int64
	if useQPS {
		// r.Duration will be -1 for endless mode. Otherwise it will have the provided duration time.
		if hasDuration {
...
func runOne(id int, funcTimes *stats.Histogram, sleepTimes *stats.Histogram, numCalls int64, start time.Time, r *periodicRunner) {
	...
	// Catch SIGINT signals from the OS into channel c
	...
	for {
		...
		if useQPS {
			...
			case <-time.After(sleepDuration):
				// continue the rest after the sleep
			}
		} else {
			// Not in fixed QPS (sleep) mode:
			select {
			case <-c:
				break MainLoop
			default:
				// continue the rest without waiting (max qps)
			}
		}
	}
	elapsed := time.Since(start)
